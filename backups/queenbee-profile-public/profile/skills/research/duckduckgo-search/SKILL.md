...
| Search returns nothing | Temporary rate limiting, poor query, or the installed `ddgs` wrapper emitting only warnings/empty stdout | Wait briefly and retry with different keywords; if CLI keeps returning empty, use DuckDuckGo HTML directly via Python stdlib (`https://html.duckduckgo.com/html/?q=...`) and parse result links/snippets before giving up |
