...
## Simple scrolling text MP4 mode

Use this lightweight path when the user provides a long list and asks for a pasteable scrolling `.mp4` rather than full ASCII art.

1. Condense the source list into one or two words per line, preserving section headers as short all-caps labels.
2. Save the text to a plain `.txt` file. Keep blank lines between sections for rhythm.
3. Render a vertical social-video MP4 with `ffmpeg drawtext`:

```bash
FONT=$(fc-match -f '%{file}' Arial)
TEXT='/path/to/scroll-words.txt'
OUT='/path/to/scroll.mp4'
ffmpeg -y -f lavfi -i color=c=#050507:s=1080x1920:r=30:d=80 \
  -vf "drawtext=fontfile=$FONT:textfile=$TEXT:fontcolor=white:fontsize=58:line_spacing=22:x=(w-text_w)/2:y=h-200-145*t:box=1:boxcolor=#05050799:boxborderw=18,format=yuv420p" \
  -c:v libx264 -preset medium -crf 18 -movflags +faststart "$OUT"
```

4. Verify the file is usable:

```bash
ffprobe -v error -show_entries stream=width,height,r_frame_rate -show_entries format=duration,size -of default=noprint_wrappers=1 "$OUT"
file "$OUT"
```

5. Deliver the video with `MEDIA:/absolute/path/to/file.mp4` and mention the saved text source path.

Pitfalls:
- OCR on generated frames can be unreliable for white-on-black scrolling text; don't treat empty OCR as proof the render failed. Use `ffprobe`, `file`, extracted frames, and visual/browser checks when available.
- Set `format=yuv420p` and `-movflags +faststart` for broad compatibility when the user wants to paste the MP4 into another project.
- If the user asks for a shorter/longer version, change both `d=` and the scroll speed in `y=`. A practical shortcut is to scale the pixel-per-second term inversely with duration, e.g. 80s uses about `145*t`, 20s uses about `550*t`, 10s uses about `1100*t` for the same long list.
- If an ffmpeg run is interrupted after `ffprobe` shows a valid duration/size, the MP4 may still be usable; verify with `ffprobe` and deliver only if the container is valid.
- If the first or last text appears too late/early, adjust the `y=` expression (`h-200-145*t`) and duration together.
